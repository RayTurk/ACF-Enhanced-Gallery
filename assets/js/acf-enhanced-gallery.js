(function ($) {
  ...
  function enhanceGalleryField($el) {
    ...
    function clearACFSelection($field) {
      // Get the ACF field instance
      var acfField = acf.getField($field);

      if (acfField) {
        // Find and trigger the close button click
        $field.find('.acf-gallery-close').trigger('click');

        // Clear the 'selected' property if it exists
        if (acfField.hasOwnProperty('selected')) {
          acfField.selected = null;
        }
      ...
      // Force the UI to update by triggering a change event
      if (acfField && acfField.$input) {
        acfField.$input().trigger('change');
      }
    }

    // Add a new function to restore ACF's default click behavior
    function restoreACFSelection($field) {
      var acfField = acf.getField($field);
      if (acfField && typeof acfField.onClickAttachment === 'function') {
        $field.find('.acf-gallery-attachment').on('click.acf', function (e) {
          // Only handle clicks if we're not in multi-select mode
          if (!selectionMode && !e.shiftKey && !e.metaKey && !e.ctrlKey) {
            acfField.onClickAttachment(e, $(this));
          }
        });
      }
    }

    // Modify the resetSelectionState function
    function resetSelectionState() {
      selectionMode = null;
      rangeStartItem = null;
      $customButtons.find('a').removeClass('active');

      // Restore ACF's default behavior
      restoreACFSelection($field);
    }

    // Button event handlers
    $customButtons.on('click', 'a.select-range', function (e) {
      e.preventDefault();
      clearACFSelection($field);  // Always clear first
      ...
      if (selectionMode === 'range') {
        ...
        selectedItems.clear();
        restoreACFSelection($field);  // Restore default behavior
      } else {
        $attachments.find('.acf-gallery-attachment').removeClass('selected');
        selectedItems.clear();
        selectionMode = 'range';
    ...
    $customButtons.on('click', 'a.bulk-delete', function (e) {
      ...
      // Check for both multi-selected and single-selected images
      var $activeItem = $field.find('.acf-gallery-attachment.active');
      var count = selectedItems.size || ($activeItem.length ? 1 : 0);
      if (count === 0) return;

      // If we have an active (single-selected) item, add it to selectedItems
      if (selectedItems.size === 0 && $activeItem.length) {
        selectedItems.add($activeItem[0]);
      }
      ...
      if (confirm(acfEnhancedGalleryL10n.deleteConfirm.replace('%d', count))) {
        ...
        // Clear the sidebar if we're deleting a single selected item
    ...
    $attachments.on('click', '.acf-gallery-attachment', function (e) {
      ...
      clearACFSelection($field); // Ensure sidebar is closed in multi-select mode
